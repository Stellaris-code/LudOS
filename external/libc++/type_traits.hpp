...
struct __lazy_and_impl<true, _Pred> : integral_constant<bool, _Pred::value> {};
...
template <class _Integer>
  constexpr typename enable_if<is_integral_v<_Integer>, uint8_t>::type &
  operator<<=(uint8_t& __lhs, _Integer __shift) noexcept
  { return __lhs = uint8_t(static_cast<unsigned char>(__lhs) << __shift); }

template <class _Integer>
  constexpr typename enable_if<is_integral_v<_Integer>, uint8_t>::type
  operator<< (uint8_t  __lhs, _Integer __shift) noexcept
  { return         uint8_t(static_cast<unsigned char>(__lhs) << __shift); }
...
template <class _Integer>
  ...
  to_integer(uint8_t __b) noexcept { return _Integer(__b); }
