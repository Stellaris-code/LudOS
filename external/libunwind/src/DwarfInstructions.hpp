...
DwarfInstructions<A, R>::evaluateExpression(pint_t expression, A &addressSpace,
  ...
  if (log)
    err("evaluateExpression(): length=%" PRIu64 "\n",
            (uint64_t)length);
  pint_t stack[100];
  pint_t *sp = stack;
  *(++sp) = initialStackValue;
  ...
  while (p < expressionEnd) {
    ...
    case DW_OP_deref_size:
      ...
      *(++sp) = value;
      if (log)
        err("sized dereference 0x%" PRIx64 "\n", (uint64_t)value);
      break;
  ...
  if (log)
    err("expression evaluates to 0x%" PRIx64 "\n", (uint64_t)*sp);
  return *sp;
}
