...
bool CFI_Parser<A>::parseInstructions(A &addressSpace, pint_t instructions,
  ...
  while ((p < instructionsEnd) && (codeOffset < pcoffset)) {
    ...
    case DW_CFA_remember_state:
      ...
      entry = (PrologInfoStackEntry *)kmalloc(sizeof(PrologInfoStackEntry));
      if (entry != NULL) {
    ...
    case DW_CFA_def_cfa:
      ...
      offset = (int64_t)addressSpace.getULEB128(p, instructionsEnd);
      if (reg > kMaxRegisterNumber) {
        err("malformed DW_CFA_def_cfa DWARF unwind, reg too big\n");
        return false;
      }
    ...
    case DW_CFA_GNU_negative_offset_extended:
      reg = addressSpace.getULEB128(p, instructionsEnd);
      if (reg > kMaxRegisterNumber) {
        err("malformed DW_CFA_GNU_negative_offset_extended DWARF "
                        "unwind, reg too big\n");
        return false;
      }
      offset = (int64_t)addressSpace.getULEB128(p, instructionsEnd)
