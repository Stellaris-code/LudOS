...
#define make_string(string, rest...) \
    ...
    len += strlen (all[cnt]);						      \
    \
    cp = result = __builtin_alloca (len);						      \
    for (cnt = 0; cnt < sizeof (all) / sizeof (all[0]); ++cnt)		      \
                ...
                if (entries != NULL)
                {
                    ...
                    if (size * 3 <= tab->n_elements * 4)
                    {
                        ...
                        struct unique_sym *newentries
                                = kcalloc (sizeof (struct unique_sym), newsize);
                        if (newentries == NULL)
                        {
nomem:
                            __rtld_lock_unlock_recursive (tab->lock);
...
#define INITIAL_NUNIQUE_SYM_TABLE 31
                    ...
                    entries = kcalloc (sizeof (struct unique_sym), size);
                    if (entries == NULL)
                        goto nomem;
                ...
                else
                {
                    ...
                    if (map->l_type == lt_loaded)
                       ...
                       setting the appropriate flag.  */
                        ((struct link_map *) map)->l_flags_1 |= 8;
                }
                ++tab->n_elements;
        ...
        if (symidx == 0 && version != NULL && version->filename != NULL
...
add_dependency (struct link_map *undef_map, struct link_map *map, int flags)
{
    ...
    if (runp != NULL)
    {
        ...
        if (__builtin_expect (l_reldepsact >= undef_map->l_reldepsmax, 0))
        {
            ...
            newp = kmalloc (sizeof (*newp) + max * sizeof (struct link_map *));
            if (newp == NULL)
            {
