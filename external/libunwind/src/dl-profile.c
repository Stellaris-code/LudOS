...
#define SCALE_1_TO_1	0x10000L
  ...
  filename = (char *) __builtin_alloca (strlen (GLRO(dl_profile_output)) + 1
                              + strlen (GLRO(dl_profile)) + sizeof ".profile");
  cp = strcpy (filename, GLRO(dl_profile_output));
  *cp++ = '/';
  strcpy (strcpy (cp, GLRO(dl_profile)), ".profile");
...
//  __profil ((void *) kcount, kcountsize, lowpc, s_scale);
  err("not goddamn supported, profiling\n");
  /* Turn on profiling.  */
  running = 1;
}
