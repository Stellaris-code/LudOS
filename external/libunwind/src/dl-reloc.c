...
{
  return (sym->st_other == STV_HIDDEN
...
void
internal_function __attribute__((noinline))
...
# error "Either TLS_TCB_AT_TP or TLS_DTV_AT_TP must be defined"
  ...
  memset (memcpy (dest, map->l_tls_initimage, map->l_tls_initimage_size),
...
#define RESOLVE_MAP(ref, version, r_type) \
    ((ELFW(ST_BIND) ((*ref)->st_info) != STB_LOCAL                              \
      &&  (!dl_symbol_visibility_binds_local_p (*ref)))              \
...
#include "dynamic-link.h"
    ...
    if ((consider_profiling)
        && l->l_info[DT_PLTRELSZ] != NULL)
      {
        ...
        l->l_reloc_result = kcalloc (sizeof (l->l_reloc_result[0]), relcount);
...
const char _itoa_lower_digits[17] = "0123456789abcdef";
