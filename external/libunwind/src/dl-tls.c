...
allocate_dtv (void *result)
{
  ...
  dtv = kcalloc (dtv_length + 2, sizeof (dtv_t));
  if (dtv != NULL)
    {
...
_dl_allocate_tls_storage (void)
{
  ...
  result = kmalloc(size);
  posix_memalign(&result, GL(dl_tls_static_align), size);
...
# error "Either TLS_TCB_AT_TP or TLS_DTV_AT_TP must be defined"
          ...
          memset (memcpy (dest, map->l_tls_initimage,
  ...
  if (listp == NULL)
    {
      ...
      listp = prevp->next = (struct dtv_slotinfo_list *)
        kmalloc (sizeof (struct dtv_slotinfo_list)
