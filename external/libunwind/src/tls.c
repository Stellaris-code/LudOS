...
init_static_tls (size_t memsz, size_t align)
{
  ...
  GL(dl_tls_static_size) = memsz + GL(dl_tls_static_size);
#if TLS_TCB_AT_TP
...
#elif TLS_DTV_AT_TP
  tcb_offset = tcbsize;
  tlsblock = kmalloc(tcb_offset + memsz + max_align
                     + TLS_PRE_TCB_SIZE + GL(dl_tls_static_size));
  tlsblock = (uint8_t*)tlsblock + TLS_PRE_TCB_SIZE;
#else
...
# error "Either TLS_TCB_AT_TP or TLS_DTV_AT_TP must be defined"
  ...
  init_static_tls (memsz, TLS_TCB_ALIGN>max_align?TLS_TCB_ALIGN:max_align);
}
