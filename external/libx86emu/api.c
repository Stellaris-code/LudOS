...
{
  x86emu_t *emu = kcalloc(1, sizeof *emu);

  emu->mem = emu_mem_new(def_mem_perm);

  emu->io.map =  kcalloc(X86EMU_IO_PORTS, sizeof *emu->io.map);
  emu->io.stats_i =  kcalloc(X86EMU_IO_PORTS, sizeof *emu->io.stats_i);
  emu->io.stats_o =  kcalloc(X86EMU_IO_PORTS, sizeof *emu->io.stats_o);
...
x86emu_t *x86emu_done(x86emu_t *emu)
{
  if(emu) {
    ...
    kfree(emu->io.map);
    kfree(emu->io.stats_i);
    kfree(emu->io.stats_o);

    kfree(emu->x86.msr);
    kfree(emu->x86.msr_perm);
...
x86emu_t *x86emu_clone(x86emu_t *emu)
{
  ...
  if(emu->log.buf && emu->log.ptr) {
    new_emu->log.buf = kmalloc(emu->log.size);
...
void x86emu_reset(x86emu_t *emu)
{
  ...
  if (x86->msr) kfree(x86->msr);
  if (x86->msr_perm) kfree(x86->msr_perm);
  ...
  x86->msr = kcalloc(X86EMU_MSRS, sizeof *x86->msr);
  x86->msr_perm = kcalloc(X86EMU_MSRS, sizeof *x86->msr_perm);
...
{
  if(emu) {
    if(emu->log.buf) kfree(emu->log.buf);
    emu->log.size = buffer_size;
    emu->log.buf = buffer_size ? kcalloc(1, buffer_size) : NULL;
...
void x86emu_log(x86emu_t *emu, const char *format, ...)
{
  ...
  if(size > 0) {
    size = kvsnprintf(emu->log.ptr, size, format, args);
    if(size > 0) {
      emu->log.ptr += size;
    }
    else {
