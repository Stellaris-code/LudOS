...
void *mem_dup(const void *src, size_t n)
{
  ...
  if(!src || !n || !(dst = kmalloc(n))) return NULL;
...
mem2_page_t *vm_get_page(x86emu_mem_t *mem, unsigned addr, int create)
{
  ...
  pdir = mem->pdir;
  if(!pdir) {
    mem->pdir = pdir = kcalloc(1, sizeof *pdir);
    // fprintf(stderr, "pdir = %p (%d)\n", pdir, sizeof *pdir);
  }

  ptable = (*pdir)[pdir_idx];
  if(!ptable) {
    ptable = (*pdir)[pdir_idx] = kcalloc(1, sizeof *ptable);
  ...
  if(create) {
    ...
    if(!page.attr) {
      page.attr = kcalloc(1, 2 * X86EMU_PAGE_SIZE);
