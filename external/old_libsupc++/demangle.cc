...
class OutputStream {
  ...
  void grow(size_t N) {
    if (N + CurrentPosition >= BufferCapacity) {
      ...
      Buffer = static_cast<char *>(krealloc(Buffer, BufferCapacity));
    }
  }
...
public:
  ...
  void printLeft(OutputStream &s) const override {
    ...
    if (static_cast<size_t>(last - first) > N) {
      ...
      int n = ksnprintf(num, sizeof(num), FloatData<Float>::spec, value);
...
class PODSmallVector {
  ...
  void reserve(size_t NewCap) {
    size_t S = size();
    if (isInline()) {
      auto* Tmp = static_cast<T*>(kmalloc(NewCap * sizeof(T)));
      std::copy(First, Last, Tmp);
      First = Tmp;
    } else
      First = static_cast<T*>(krealloc(First, NewCap * sizeof(T)));
    Last = First + S;
    Cap = First + NewCap;
  }
...
__cxa_demangle(const char *mangled_name, char *buf, size_t *n, int *status) {
    ...
    size_t len = strlen(mangled_name);
    ...
    if (internal_status == success)
    {
        ...
        {
            internal_size = 1024;
            buf = static_cast<char*>(kmalloc(internal_size));
        }
