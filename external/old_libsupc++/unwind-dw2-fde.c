...
__register_frame (void *begin)
{
  ...
  ob = (struct object*)kmalloc (sizeof (struct object));
...
{
  struct object *ob = kmalloc (sizeof (struct object));
...
__deregister_frame (void *begin)
{
  /* If .eh_frame is empty, we haven't registered.  */
  if (*(uword *) begin != 0)
    kfree (__deregister_frame_info (begin));
}
...
start_fde_sort (struct fde_accumulator *accu, size_t count)
{
  ...
  if ((accu->linear = kmalloc (size)))
    {
      accu->linear->count = 0;
      if ((accu->erratic = kmalloc (size)))
	   ...
	   struct fde_vector *linear, struct fde_vector *erratic)
{
  ...
  assert (sizeof (const fde *) == sizeof (const fde **));
...
end_fde_sort (struct object *ob, struct fde_accumulator *accu, size_t count)
{
  ...
  assert (!accu->linear || accu->linear->count == count);
  ...
  if (accu->erratic)
    {
      ...
      assert (accu->linear->count + accu->erratic->count == count);
    ...
    {
      /* We've not managed to kmalloc an erratic array,
