...
/* ??? For the public function interfaces, we tend to assert that the
...
_Unwind_GetGR (struct _Unwind_Context *context, int index)
{
  ...
  assert (index < (int) sizeof(dwarf_reg_size_table));
  ...
  if (size == sizeof(_Unwind_Ptr))
    ...
    {
      assert (size == sizeof(_Unwind_Word));
      return * (_Unwind_Word *) (_Unwind_Internal_Ptr) val;
    }
}
            ...
            _Unwind_Word val)
{
  ...
  assert (dwarf_reg_size_table[index] <= sizeof (_Unwind_Context_Reg_Val));
          ...
          struct _Unwind_Context *context, _Unwind_Word initial)
{
  ...
  while (op_ptr < op_end)
    {
      enum dwarf_location_atom op = static_cast<dwarf_location_atom>(*op_ptr++);
      _Unwind_Word result;
      _uleb128_t reg, utmp;
    ...
    default:
      ...
      assert ((size_t) stack_elt < sizeof(stack)/sizeof(*stack));
...
execute_cfa_program (const unsigned char *insn_ptr,
             ...
             _Unwind_FrameState *fs)
{
  frame_state_reg_info *unused_rs = NULL;
    ...
    case DW_CFA_remember_state:
      {
        ...
        unused_rs = unused_rs->prev;
          ...
          new_rs = (frame_state_reg_info*)__builtin_alloca (sizeof (struct frame_state_reg_info));
              ...
              struct _Unwind_Context *target)
{
  ...
  for (i = 0; i < __LIBGCC_DWARF_FRAME_REGISTERS__; ++i)
      ...
      if (target->by_value[i] && c)
    {
      ...
      if (dwarf_reg_size_table[i] == sizeof (_Unwind_Word))
        ...
        {
          assert (dwarf_reg_size_table[i] == sizeof (_Unwind_Ptr));
  ...
  if (!_Unwind_GetGRPtr (current, __builtin_dwarf_sp_column ()))
    ...
    return ((uint8_t*)target_cfa - (uint8_t*)current->cfa) + target->args_size;
      else
    return ((uint8_t*)current->cfa - (uint8_t*)target_cfa) - target->args_size;
    }
  return 0;
}
