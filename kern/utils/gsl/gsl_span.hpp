...
#include <utils/gsl/gsl_byte.hpp>   // for byte
#include <utils/gsl/gsl_util.hpp>   // for narrow_cast, narrow

#include <algorithm.hpp> // for lexicographical_compare
...
constexpr const ptrdiff_t dynamic_extent = -1;
...
namespace details
{
    ...
    template <ptrdiff_t From, ptrdiff_t To>
...
private:
    ...
    template <ptrdiff_t CallerExtent>
...
template <class ElementType, ptrdiff_t FirstExtent, ptrdiff_t SecondExtent>
...
template <class Ptr>
span<typename Ptr::element_type> make_span(Ptr& cont, ptrdiff_t count)
{
...
template <class ElementType, ptrdiff_t Extent>
inline constexpr ElementType& at(span<ElementType, Extent> s, ptrdiff_t index)
{
