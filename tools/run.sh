...
./strip-bin.sh
cd ..
qemu-system-i386 -netdev user,id=n0 -device rtl8139,netdev=n0,mac=DE:AD:BE:EF:12:34 -d mmu,cpu_reset,guest_errors -soundhw pcspk -vga std -serial stdio -no-reboot -m 72M -kernel build/bin/LudOS.bin -hdd build/bin/LudOS.img -initrd 'build/bin/initrd.tar initrd,build/bin/stripped.bin kernel_binary'
