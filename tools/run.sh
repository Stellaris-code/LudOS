...
./strip-bin.sh
cd ..
qemu-system-i386 -s -d mmu,cpu_reset,guest_errors,int -soundhw pcspk -vga std -serial stdio -no-reboot -m 128M -kernel build/bin/LudOS.bin -append "loglevel=debug" -initrd 'build/bin/initrd.tar initrd,build/bin/stripped.bin kernel_binary' -drive id=disk,file=build/bin/LudOS.img,if=none -device ide-hd,drive=disk,bus=ide.0,unit=0 -enable-kvm
